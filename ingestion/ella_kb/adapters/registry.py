...
from ..core.config import EllaKbConfig
...
def load_external_adapters() -> None:
    """Discover out-of-tree adapters declared via the `ella_kb.adapters` entry point."""
    try:
        eps = md.entry_points(group="ella_kb.adapters")
    except TypeError:  # older importlib API
        eps = md.entry_points().get("ella_kb.adapters", [])  # type: ignore[attr-defined]
    for ep in eps:
        ep.load()  # importing the target fires its @register


def build_enabled_adapters(cfg: EllaKbConfig, ctx: AdapterContext) -> dict[str, SourceAdapter]:
