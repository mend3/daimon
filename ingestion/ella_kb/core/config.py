...
class EllaKbConfig(BaseModel):
    ledger_path: str = "~/.hermes/ella_kb/ledger.db"
...
def load_config(path: str | None = None) -> EllaKbConfig:
    """Load from an explicit path, $ELLA_KB_CONFIG, or ~/.hermes/ella_kb.yaml.
    ...
    candidate = path or os.environ.get("ELLA_KB_CONFIG") or "~/.hermes/ella_kb.yaml"
    p = Path(candidate).expanduser()
    raw: dict[str, Any] = {}
    if p.exists():
        raw = _expand(yaml.safe_load(p.read_text()) or {})
        if isinstance(raw, dict) and "ella_kb" in raw:
            raw = raw["ella_kb"]
    cfg = EllaKbConfig(**raw)
