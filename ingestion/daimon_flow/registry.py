...
def load_external_nodes() -> None:
    try:
        eps = md.entry_points(group="daimon_flow.nodes")
    except TypeError:
        eps = md.entry_points().get("daimon_flow.nodes", [])  # type: ignore[attr-defined]
    for ep in eps:
        ep.load()
...
def ensure_loaded() -> None:
    ...
    if _loaded:
        return
    import daimon_flow.nodes  # noqa: F401  (built-in registration via @node)
    load_external_nodes()
    _loaded = True
